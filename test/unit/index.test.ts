...
describe('plugin entry point', () => {
  it('exports the named plugin, parser wrappers, and its mode option with a default', () => {
    ...
    expect(JSON_UTILS_MODES).toEqual(['sort-keys', 'package-json', 'vscode-settings']);
    expect(options[JSON_UTILS_MODE_OPTION]?.choices.map((choice) => choice.value)).toEqual(JSON_UTILS_MODES);
    expect(options[JSON_UTILS_MODE_OPTION]?.default).toBe('sort-keys');
